...
This site is Ukrainian-only, so `uk` is both the sole locale and the default. The i18n machinery is still in place — adding a second locale means adding its code here and a matching catalog (the `add-locale` agent skill walks through it).

`as-needed` strips the locale segment for the default locale, so with a single locale every URL is unprefixed: `/about`, not `/uk/about`.
...
Request config loads the matching JSON catalog in `apps/ui/src/lib/i18n.ts`. The default timezone is `Europe/Kyiv`.

Message keys are typed through `apps/ui/src/types/global.d.ts`, using `apps/ui/locales/uk.json` as the source shape. This gives autocomplete for `useTranslations()` and `getTranslations()`.
