...
How Strapi content updates become visible on the Next.js frontend without a rebuild.
...
Next.js revalidation marks the application cache stale. It does not automatically evict an upstream CDN — that has to be done through the CDN's own tooling.
:::
...
`STRAPI_REVALIDATE_SECRET` must match in Strapi and the UI. If it is missing or mismatched, UI revalidation rejects the request with `401`.
:::
...
See [Strapi environment variables](../strapi/environment-variables) and [UI environment variables](../ui/environment-variables) for the complete environment reference.
