...
activates only once its environment variables are set. The bundled provider is
**Sentry** (error tracking, UI only).
...
`withSpan` builds on the OpenTelemetry API, so any OpenTelemetry-compatible
backend (Grafana, Datadog, an OTLP collector, …) can be added as a provider. The
`logger` / `logError` / `withSpan` API the apps call is backend-agnostic and does
not change with the target.

Strapi's provider registry (`apps/strapi/src/telemetry/`) is currently empty —
the scaffold is kept so a backend can be plugged in without rewiring startup.
:::

## Sentry

[Sentry](https://sentry.io) is an error monitoring service used to capture runtime exceptions, handled errors, and release diagnostics.
...
- logs print to the devtools console only; **nothing is exported to a backend**
  (telemetry providers are server-only),
